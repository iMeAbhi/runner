...
export default function Background() {
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden" style={{ backgroundImage: 'var(--bg-image)', backgroundSize: '200% 200%' }}>
      ...
      <motion.div
        className="ambient-bloom absolute -top-32 -left-24 h-96 w-96 rounded-full blur-3xl"
        style={{ background: 'rgb(var(--accent) / 0.35)' }}
        animate={{ x: [0, 40, -20, 0], y: [0, 30, 60, 0] }}
        transition={{ duration: 26, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="ambient-bloom absolute bottom-0 right-0 h-[28rem] w-[28rem] rounded-full blur-3xl"
